...
describe('Town Selection - depends on Part 1 passing', () => {
  ...
  beforeEach(async () => {
    ...
    fileUpload = renderData.getByTestId('npcUpload');
    newTownButton = renderData.getByTestId('newTownButton');
  });

  describe('Part 3 - Creating a new town', () => {

    const createTownWithOptions = async (params: { townName: string, userName: string, togglePublicBox?: boolean, townID?: string, roomPassword?: string, errorMessage?: string, npcs: NPC[] }) => {
    ...
    describe('when clicking create', () => {
      ...
      describe('with valid values', () => {
        ...
        const TEST_SCRIPT: Script = new Script("id", ["line 1"], true);
        const TEST_PATH: Path = new Path("id", [randomLocation()]);
        const TEST_BEHAVIOR: Behavior = new Behavior("id", "name", TEST_SCRIPT, TEST_PATH);

        const NPC1: NPC = new NPC("1", "npc1", randomLocation(), randomLocation(), TEST_BEHAVIOR, "spriteImage", "spriteJSON");
        const NPC2: NPC = new NPC("2", "npc2", randomLocation(), randomLocation(), TEST_BEHAVIOR, "spriteImage", "spriteJSON");

        const NPC_ARRAY_LENGTH_2: NPC[] = [NPC1, NPC2];
        ...
        it('calls createTown on the apiClient with the provided values (public town)', async () => {
          ...
          await waitFor(() => expect(mockCreateTown)
            .toHaveReturnedWith({
              coveyRoomPassword: roomPassword,
              coveyTownID: townID
            }));
        });
        ...
        it('calls createTown on the apiClient with the provided values (public town) with 2 npcs', async () => {
          const townID = nanoid();
          const roomPassword = nanoid();
          const townName = nanoid();
          await createTownWithOptions({
        ...
        it('calls createTown on the apiClient with the provided values (not public town) with 2 npcs', async () => {
          const townID = nanoid();
          const roomPassword = nanoid();
          const townName = nanoid();
          await createTownWithOptions({

...
function App(props: { setOnDisconnect: Dispatch<SetStateAction<Callback | undefined>> }) {
  ...
  const [npcMovementCallbacks] = useState<NPCMovementCallback[]>([]);
  const [playersInTown, setPlayersInTown] = useState<Player[]>([]);
  const [npcsInTown, setNPCsInTown] = useState<NPC[]>([]);
  ...
  const setupGameController = useCallback(
    async (initData: TownJoinResponse) => {
      ...
      const localNPCs = initData.currentNPCs.map(sp => NPC.fromServerNPC(sp));
      setNPCsInTown(localNPCs);
      ...
      socket.on('npcLocationUpdated', (npc: ServerNPC) => {
        const myNPC = localNPCs.find(c => c.id === npc._id);
        if (myNPC) {
          myNPC.updateLocationFromServerNPC(npc);
          npcMovementCallbacks.forEach(cb => cb(npc));
        }
      });
      socket.on('npcUpdated', (npc: ServerNPC) => {
        const myNPC = localNPCs.find(c => c.id === npc._id);
        if (myNPC) {
          myNPC.updateFromServerNPC(npc);
          npcMovementCallbacks.forEach(cb => cb(npc));
        }
      });
    ...
    [playerMovementCallbacks, npcMovementCallbacks],
  );

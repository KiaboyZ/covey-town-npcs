...
export interface TownJoinResponse {
  ...
  /** List of npcs currently in this town * */
  currentNPCs: NPC[];
...
export interface NPCListRequest {
  coveyTownID: string;
  sessionToken: string;
}
...
export interface NPCAddRequest {
  coveyTownID: string;
  sessionToken: string;
  npc: RequestNPC;
}
...
export async function townJoinHandler(requestData: TownJoinRequest): Promise<ResponseEnvelope<TownJoinResponse>> {
  ...
  return {
    ...
    response: {
      ...
      currentNPCs: coveyTownController.npcs,
...
export function townCreateHandler(requestData: TownCreateRequest): ResponseEnvelope<TownCreateResponse> {
  ...
  const npcs = requestData.npcs.map((npc) => NPC.fromRequest(npc as RequestNPC));
  const newTown = townsStore.createTown(requestData.friendlyName, requestData.isPubliclyListed, npcs);
  return {
    isOK: true,
    response: {
...
export function npcListHandler(_requestData: NPCListRequest): ResponseEnvelope<NPCListResponse> {
  const townsStore = CoveyTownsStore.getInstance();
  const townController = townsStore.getControllerForTown(_requestData.coveyTownID);
  if (!townController?.getSessionByToken(_requestData.sessionToken)) {
    return {
      isOK: false, message: 'invalid town ID or session token',
    };
  }
  return {
    isOK: true, response: { npcs: townController.npcs },
  };
}

export function npcAddHandler(_requestData: NPCAddRequest): ResponseEnvelope<null> {
  const townsStore = CoveyTownsStore.getInstance();
  const townController = townsStore.getControllerForTown(_requestData.coveyTownID);
  if (!townController?.getSessionByToken(_requestData.sessionToken)) {
    return {
  ...
  townController.addNPCPlayer(NPC.fromRequest(_requestData.npc));
  return {
    isOK: true, message: 'successfully added NPC',
  };
}

export function npcUpdateHandler(_requestData: NPCUpdateRequest): ResponseEnvelope<Record<string, null>> {
  const townsStore = CoveyTownsStore.getInstance();
  const townController = townsStore.getControllerForTown(_requestData.coveyTownID);
  if (!townController?.getSessionByToken(_requestData.sessionToken)) {
    return {
      isOK: false, message: 'invalid town ID or session token', response: {},
    };
  }
  const id = _requestData.npc._id || _requestData.npc.id;
  const result = townController.updateNPC(NPC.fromRequest(_requestData.npc), id);
  if (result) {
    return {
      isOK: true, message: 'successfully updated NPC', response: {},
    };
  }
  return {
    isOK: false, message: 'did not update NPC', response: {},
  };
}
...
function townSocketAdapter(socket: Socket): CoveyTownListener {
  return {
    onNPCUpdate(updatedNPC: NPC): void {
      socket.emit('npcUpdated', updatedNPC);
    },
    onNPCLocationUpdate(updatedNPC: NPC): void {
      socket.emit('npcLocationUpdated', updatedNPC);
    },

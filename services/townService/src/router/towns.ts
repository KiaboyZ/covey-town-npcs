...
import {
  ...
  npcAddHandler,
  npcListHandler, npcUpdateHandler,
...
export default function addTownRoutes(http: Server, app: Express): io.Server {
  ...
  app.post('/towns/:townID/npc', express.json(), async (req, res) => {
    try {
      const result = npcAddHandler({
        coveyTownID: req.params.townID,
        sessionToken: req.body.sessionToken,
        npc: req.body.npc,
      });
  ...
  app.patch('/towns/:townID/npc/', express.json(), async (req, res) => {
    try {
      const result = npcUpdateHandler({
        coveyTownID: req.params.townID,
        sessionToken: req.body.sessionToken,
        npc: req.body.npc,
      });
  ...
  app.get('/towns/:townID/npcs', express.json(), async (req, res) => {
    try {
      const result = npcListHandler({
        coveyTownID: req.params.townID,
        sessionToken: req.body.sessionToken,
      });
      res.status(StatusCodes.OK)

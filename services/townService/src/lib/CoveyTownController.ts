...
import NPCRunner from '../types/NPCRunner';
...
export default class CoveyTownController {
  ...
  constructor(friendlyName: string, isPubliclyListed: boolean, npcs: NPC[]) {
    ...
    this._npcs = npcs;
    const cb = (npc: NPC) => this._listeners.forEach(listener => listener.onNPCLocationUpdate(npc));
    this._npcRunners = this._npcs.map((npc)=>new NPCRunner(npc, 1000, cb));
  }
  ...
  /**
   * Adds an NPC player to this Covey Town and returns the player
   *
   * @param newNPC The new player to add to the town
   */
  addNPCPlayer(newNPC: NPC):NPC {
  ...
  /**
   * Updates an NPC player to this Covey Town and returns the player
   *
   * @param updatedNPC The updated player to add to the town
   * @param npcID the ID of the NPC
   */
  updateNPC(updatedNPC: NPC, npcID: string):boolean {
    const npc = this._npcs.find((_npc) => _npc.id === npcID);
    if (npc) {
      if (updatedNPC.name) {
        npc.name = updatedNPC.name;
      }
      if (updatedNPC.currentLocation) {
        npc.currentLocation = updatedNPC.currentLocation;
      }
      if (updatedNPC.startLocation) {
        npc.startLocation = updatedNPC.startLocation;
      }
      if (updatedNPC.behavior) {
        npc.behavior = updatedNPC.behavior;
      }
      if (updatedNPC.spriteImage) {
        npc.spriteImage = updatedNPC.spriteImage;
      }
      if (updatedNPC.spriteJSON) {
        npc.spriteJSON = updatedNPC.spriteJSON;
      }
      this._listeners.forEach(listener => listener.onNPCUpdate(npc));
      return true;
    }
    return false;
  }
   ...
   * @param npcID the ID of the NPC
   */
  updateNPCLocation(updatedNPC: NPC, npcID: string):boolean {
    const npc = this._npcs.find((_npc) => _npc.id === npcID);
    if (npc) {
      npc.currentLocation = updatedNPC.currentLocation;
      npc.startLocation = updatedNPC.startLocation;

      this._listeners.forEach(listener => listener.onNPCLocationUpdate(npc));
      return true;
    }
    return false;
  }
